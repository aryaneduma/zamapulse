...
import { SEASON_1_RANKED, SEASON_1_CHOICE_AWARDS, SEASON_1_STATS } from '../data/season1';
import { SEASON_2_RANKED, SEASON_2_CHOICE_AWARDS, SEASON_2_STATS } from '../data/season2';
import { SEASON_3_RANKED, SEASON_3_CHOICE_AWARDS, SEASON_3_STATS } from '../data/season3';
import { SEASON_4_RANKED, SEASON_4_CHOICE_AWARDS, SEASON_4_STATS } from '../data/season4';
...
const Leaderboard: React.FC = () => {
    ...
    const loadMore = async (pageNum = page, season = selectedSeason) => {
        if (['season1', 'season2', 'season3', 'season4'].includes(season)) return; // Handled statically
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        if (isStaticSeason) {
            ...
            if (selectedSeason === 'season1') {
                choiceAwards = SEASON_1_CHOICE_AWARDS;
                rankedList = SEASON_1_RANKED;
            } else if (selectedSeason === 'season2') {
                choiceAwards = SEASON_2_CHOICE_AWARDS;
                rankedList = SEASON_2_RANKED;
            } else if (selectedSeason === 'season3') {
                choiceAwards = SEASON_3_CHOICE_AWARDS;
                rankedList = SEASON_3_RANKED;
            } else if (selectedSeason === 'season4') {
                choiceAwards = SEASON_4_CHOICE_AWARDS;
                rankedList = SEASON_4_RANKED;
            }

            // Check Choice Awards
            const choiceMatch = choiceAwards.find(
                u => (u.handle && u.handle.toLowerCase().includes(verifyQuery.toLowerCase())) || 
            ...
            const rankedMatch = rankedList.find(
                u => u.url.toLowerCase().includes(verifyQuery.toLowerCase()) || 
                     u.name.toLowerCase().includes(verifyQuery.toLowerCase())
            );
    ...
    return (
        <div className="w-full max-w-5xl flex flex-col items-center animate-in fade-in duration-500">
            ...
            {selectedSeason === 'season1' ? (
                renderStaticSeason(SEASON_1_STATS, SEASON_1_CHOICE_AWARDS, SEASON_1_RANKED)
            ) : selectedSeason === 'season2' ? (
                renderStaticSeason(SEASON_2_STATS, SEASON_2_CHOICE_AWARDS, SEASON_2_RANKED)
            ) : selectedSeason === 'season3' ? (
                renderStaticSeason(SEASON_3_STATS, SEASON_3_CHOICE_AWARDS, SEASON_3_RANKED)
            ) : selectedSeason === 'season4' ? (
                renderStaticSeason(SEASON_4_STATS, SEASON_4_CHOICE_AWARDS, SEASON_4_RANKED)
            ) : (
